...
### 実装の役割分担（Codex 実装 → Opus 5 ＋ Antigravity 並行レビュー → Codex 修正 → Fable 5 最終レビュー）

- **実装は Codex に委譲する**（codex プラグインの rescue サブエージェント経由）。Claude は §12 の該当 PR の受け入れ条件・テストケース・対象ファイルをまとめた指示を Codex に渡す。Codex が使えない場合や第二実装案が欲しい場合は、antigravity サブエージェント（Antigravity CLI `agy` / Gemini 3。定義は `.claude/agents/antigravity.md`）に実装を委譲してよい
- **一次レビューは Opus 5 サブエージェントと antigravity サブエージェントが並行して行う**（Agent ツールで両者を同時に起動する。Opus 5 は `model: opus` を指定し serena MCP のシンボル検索・参照解析ツールを使わせる。antigravity には読み取り専用でレビューさせる）。憲法（INV/ARCH）違反・受け入れ条件の漏れ・規約違反（型注釈、`tr()`、テスト追加）を重点的に確認
- **レビューで見つかった問題の修正は Codex に差し戻す**（両レビューの指摘を重複排除してまとめ、rescue サブエージェントに渡す）
- **Codex の修正後、最終レビューはメインセッション（Fable 5）が直接行う**。問題がなければ `bash tools/checks.sh` を通してからマージ手順（上記 4）へ進む
