...
### 実装の役割分担（Codex 実装 → Claude レビュー・修正）

- **実装は Codex に委譲する**（codex プラグインの rescue サブエージェント経由）。Claude は §12 の該当 PR の受け入れ条件・テストケース・対象ファイルをまとめた指示を Codex に渡す
- **Codex の成果物は Claude が必ずレビューする**。憲法（INV/ARCH）違反・受け入れ条件の漏れ・規約違反（型注釈、`tr()`、テスト追加）を重点的に確認
- **レビューで見つかった問題の修正は Claude が行う**。修正後に `bash tools/checks.sh` を通してからマージ手順（上記 4）へ進む
