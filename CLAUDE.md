...
### 実装の役割分担（Codex 実装 → Opus 5 レビュー → Codex 修正 → Fable 5 最終レビュー）

- **実装は Codex に委譲する**（codex プラグインの rescue サブエージェント経由）。Claude は §12 の該当 PR の受け入れ条件・テストケース・対象ファイルをまとめた指示を Codex に渡す
- **一次レビューは Opus 5 サブエージェントが行う**（Agent ツールで `model: opus` を指定し、serena MCP のシンボル検索・参照解析ツールを使わせる）。憲法（INV/ARCH）違反・受け入れ条件の漏れ・規約違反（型注釈、`tr()`、テスト追加）を重点的に確認
- **レビューで見つかった問題の修正は Codex に差し戻す**（指摘内容をまとめて rescue サブエージェントに渡す）
